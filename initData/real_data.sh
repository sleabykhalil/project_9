...
curl -d "patId=5&e=Le patient déclare qu'il « se sent très bien »
Poids égal ou inférieur au poids recommandé" -X POST http://localhost:8082/patHistory/add

curl -d "patId=5&e=Le patient déclare qu'il se sent fatigué pendant la journée
...
curl -d "patId=5&e=Le patient déclare qu'il ne se sent pas si fatigué que ça
...
curl -d "patId=6&e=Le patient déclare qu'il ressent beaucoup de stress au travail
Il se plaint également que son audition est anormale dernièrement" -X POST http://localhost:8082/patHistory/add

curl -d "patId=6&e=Le patient déclare avoir fait une réaction aux médicaments au cours des 3 derniers mois
...
curl -d "patId=6&e=Le patient déclare que tout semble aller bien
...
curl -d "patId=7&e=Le patient déclare qu'il est fumeur et qu'il a cessé de fumer l'année dernière
...
curl -d "patId=8&e=Le patient déclare qu'il lui est devenu difficile de monter les escaliers
...
curl -d "patId=8&e=Le patient déclare avoir commencé à fumer depuis peu
Hémoglobine A1C supérieure au niveau recommandé" -X POST http://localhost:8082/patHistory/add

curl -d "patId=9&e=Le patient déclare avoir des douleurs au cou occasionnellement
...
curl -d "patId=9&e=Le patient déclare avoir eu plusieurs épisodes de vertige depuis la dernière visite.
Taille incluse dans la fourchette concernée" -X POST http://localhost:8082/patHistory/add

curl -d "patId=9&e=Le patient déclare qu'il souffre encore de douleurs cervicales occasionnelles
...
curl -d "patId=10&e=Le patient déclare qu'il se sent bien
Poids corporel supérieur au poids recommandé" -X POST http://localhost:8082/patHistory/add

curl -d "patId=10&e=Le patient déclare qu'il se sent bien" -X POST http://localhost:8082/patHistory/add

curl -d "patId=11&e=Le patient déclare qu'il se réveille souvent avec une raideur articulaire
Il se plaint également de difficultés pour s’endormir
Poids corporel supérieur au poids recommandé
Tests de laboratoire indiquant un taux de cholestérol LDL élevé" -X POST http://localhost:8082/patHistory/add

curl -d "patId=12&e=Les tests de laboratoire indiquent que les anticorps sont élevés
Hémoglobine A1C supérieure au niveau recommandé" -X POST http://localhost:8082/patHistory/add

curl -d "patId=13&e=Le patient déclare avoir de la difficulté à se concentrer sur ses devoirs scolaires
Hémoglobine A1C supérieure au niveau recommandé" -X POST http://localhost:8082/patHistory/add

curl -d "patId=13&e=Le patient déclare qu'il s’impatiente facilement en cas d’attente prolongée
...
curl -d "patId=13&e=Le patient signale qu'il est facilement irrité par des broutilles
...
curl -d "patId=14&e=Le patient déclare qu'il n'a aucun problème
